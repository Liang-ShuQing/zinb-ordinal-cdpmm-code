############################################################
# Real-data analysis (legacy combined driver)
# Prefer the simulation-aligned programs:
#   prog1_cdpmm_joint_vs_separate.R  — CDPMM joint vs separate
#   prog2_cdpmm_vs_gaussian_joint.R  — CDPMM joint vs Gaussian joint
# This file remains usable with RE_PRIOR=cdpmm|gaussian and FIT_SEPARATE.
# Notation: y1=ZINB(HSPNIT), y2=ordinal(SHLT); b=(zero,count,ordinal)
...
covariate_vars <- c("SMOKEV", "HIBP", "DIAB", "LUNG")
...
re_prior <- tolower(.env_chr("RE_PRIOR", re_prior))
if (!re_prior %in% c("cdpmm", "gaussian")) {
  stop("RE_PRIOR must be 'cdpmm' or 'gaussian', got: ", re_prior)
}
G_mix <- .env_int("G_MIX", G_mix)
fit_separate <- .env_int("FIT_SEPARATE", fit_separate)
...
ordinal_export <- c(
  ...
  "init_cdpmm_1d", "update_cdpmm_1d", "stick_break_weights", "safe_riwish", "safe_solve",
...
# 7. Save results
############################################################
rds_tag <- paste0("MCMC_Analysis_Results_", re_prior, ".rds")
...
write.csv(diag_table, file.path(OUT_DIR, "diagnose_rhat_table.csv"), row.names = FALSE)
write.csv(alpha_results, file.path(OUT_DIR, "Posterior_Joint_Alpha_RealData.csv"), row.names = FALSE)
write.csv(beta_results, file.path(OUT_DIR, "Posterior_Joint_Beta_RealData.csv"), row.names = FALSE)
write.csv(gamma_results, file.path(OUT_DIR, "Posterior_Joint_Gamma_RealData.csv"), row.names = FALSE)
write.csv(delta_results, file.path(OUT_DIR, "Posterior_Joint_Delta_RealData.csv"), row.names = FALSE)
write.csv(r_results, file.path(OUT_DIR, "Posterior_Joint_r_RealData.csv"), row.names = FALSE)
write.csv(sigma_results, file.path(OUT_DIR, "Posterior_Joint_SigmaDiag_RealData.csv"), row.names = FALSE)
write.csv(rho_results, file.path(OUT_DIR, "Posterior_Joint_Rho_RealData.csv"), row.names = FALSE)
if (!is.null(variance_ratio_table)) {
  write.csv(variance_ratio_table, file.path(OUT_DIR, "variance_ratio_realdata.csv"), row.names = FALSE)
}
if (!is.null(model_compare)) {
  write.csv(model_compare, file.path(OUT_DIR, "model_compare_realdata.csv"), row.names = FALSE)
}

cat("\n已保存 RDS:", rds_path, "\n")
cat("CSV 与诊断表已写入:", OUT_DIR, "\n")
cat("绘图: 运行 plot_diagnostics.R\n")
cat("=== 分析完成 (n=", n, ", N=", N, ") ===\n", sep = "")
